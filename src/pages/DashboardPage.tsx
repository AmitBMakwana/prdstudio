...
import { 
  ...
  Layers, Terminal, LayoutGrid, List, Copy, Check, Filter, FolderKanban, Activity
} from 'lucide-react';
  ...
  return (
    <div className="container animate-fade-in" style={{ paddingBottom: '60px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          ...
          <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginTop: '4px' }}>
            Manage your project canvases, inspect 6-file specification packages, and copy Master AI prompts.
          </p>
        </div>
        ...
        >
          <Plus size={18} /> Create New PRD
        </button>
      </div>

      {/* 4 KPI Metric Cards Row (Points 8 & 9) */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '28px' }}>
        {/* Metric 1: Projects */}
        <div className="canvas-card" style={{ padding: '20px', cursor: 'default', borderRadius: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <span style={{ fontSize: '12px', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase' }}>PROJECTS</span>
            <div style={{ background: 'var(--primary-light)', padding: '6px', borderRadius: '8px', color: 'var(--primary)' }}>
              <FolderKanban size={18} />
            </div>
          </div>
          <div style={{ fontSize: '28px', fontWeight: 800, color: 'var(--text-primary)' }}>
            {prds.length} <span style={{ fontSize: '12px', color: '#10B981', fontWeight: 700 }}>+3 this month</span>
          </div>
        </div>
        ...
        <div className="canvas-card" style={{ padding: '20px', cursor: 'default', borderRadius: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <span style={{ fontSize: '12px', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase' }}>AI CREDITS</span>
            <div style={{ background: 'var(--primary-light)', padding: '6px', borderRadius: '8px', color: 'var(--primary)' }}>
              <Zap size={18} />
            </div>
          </div>
        ...
        {/* Metric 3: PRDs Generated */}
        <div className="canvas-card" style={{ padding: '20px', cursor: 'default', borderRadius: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <span style={{ fontSize: '12px', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase' }}>PRDS GENERATED</span>
            <div style={{ background: 'var(--primary-light)', padding: '6px', borderRadius: '8px', color: 'var(--primary)' }}>
              <FileText size={18} />
            </div>
          </div>
          <div style={{ fontSize: '28px', fontWeight: 800, color: 'var(--text-primary)' }}>
            {prds.length * 6} <span style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600 }}>Spec Files</span>
          </div>
        </div>

        {/* Metric 4: AI Engine Status (Replaces raw database box) */}
        <div className="canvas-card" style={{ padding: '20px', cursor: 'default', borderRadius: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <span style={{ fontSize: '12px', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase' }}>AI ENGINE STATUS</span>
            <span style={{ color: '#10B981', fontSize: '12px', fontWeight: 800, background: '#D1FAE5', padding: '2px 8px', borderRadius: '10px' }}>● Operational</span>
          </div>
          <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>
            Generation API Active <span style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 500 }}>(~12 sec avg)</span>
          </div>
        </div>
      </div>

      {/* Control Bar: Search Input, Platform Filters & View Switcher */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '12px', padding: '2px' }}>
          <button
            ...
            style={{ padding: '6px 12px', borderRadius: '10px', background: viewMode === 'grid' ? 'var(--primary-light)' : 'transparent', color: viewMode === 'grid' ? 'var(--primary)' : 'var(--text-muted)', border: 'none', cursor: 'pointer' }}
          ...
          <button
            ...
            style={{ padding: '6px 12px', borderRadius: '10px', background: viewMode === 'table' ? 'var(--primary-light)' : 'transparent', color: viewMode === 'table' ? 'var(--primary)' : 'var(--text-muted)', border: 'none', cursor: 'pointer' }}
      ...
      {filteredPRDs.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px' }}>
          {filteredPRDs.map((prd, index) => {
            ...
            return (
              ...
              >
                {/* Header Banner with Floating Badge */}
                ...
                <div className="canvas-card-body">
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                      ...
                      <span style={{ background: '#D1FAE5', color: '#065F46', padding: '2px 8px', borderRadius: '10px', fontSize: '10px', fontWeight: 800 }}>
                        ● Completed
                      </span>
                    </div>
                  ...
                  <div style={{ marginTop: '24px', paddingTop: '16px', borderTop: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <button
                        ...
                        style={{ padding: '6px 10px', borderRadius: '8px', background: 'var(--bg-main)', border: '1px solid var(--border-color)', color: 'var(--text-secondary)', fontSize: '12px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer' }}
                        title="Copy Master AI Prompt"
                      >
                      ...
                      <button
                        ...
                        style={{ padding: '6px 14px', borderRadius: '8px', background: 'var(--primary-light)', color: 'var(--primary)', fontSize: '12px', fontWeight: 700, border: '1px solid var(--border-hover)', cursor: 'pointer' }}
                      >
                        Open Project →
                      </button>
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          if (confirm(`Delete project "${prd.title}"?`)) onDeletePRD(prd.id);
                        }}
                        style={{ padding: '6px 8px', borderRadius: '8px', background: '#FEF2F2', border: '1px solid #FCA5A5', color: '#EF4444', fontSize: '12px', cursor: 'pointer' }}
        ...
        /* TABLE LIST VIEW WITH PROJECT STATUS BADGES (Point 10 & 11) */
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid var(--border-color)', color: 'var(--text-muted)' }}>
                ...
                <th style={{ padding: '12px 16px', fontWeight: 700 }}>Updated Date</th>
              ...
              {filteredPRDs.map(prd => (
                <tr key={prd.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    <span style={{ background: '#D1FAE5', color: '#065F46', padding: '3px 10px', borderRadius: '10px', fontSize: '11px', fontWeight: 800 }}>
                      ● Completed
                    </span>
                  </td>
                  <td style={{ padding: '14px 16px', color: 'var(--text-muted)' }}>
